...
# ======================
# STYLE VISUEL GLOBAL (sans modifier les sliders)
# ======================
st.markdown("""
<style>
.stApp {
...
TRADUCTION_FEATURES = {
    ...
    "AMT_INCOME_TOTAL": "Capacité financière du client",
    "DAYS_BIRTH": "Âge du client",
    "DAYS_EMPLOYED": "Stabilité professionnelle",
    "DAYS_REGISTRATION": "Stabilité administrative",
    "DAYS_ID_PUBLISH": "Ancienneté administrative",
    "NAME_FAMILY_STATUS_Married": "Situation familiale",
    "OCCUPATION_TYPE_Laborers": "Type d’emploi",
    "REGION_POPULATION_RELATIVE": "Risque lié à la zone de résidence",
    "REGION_RATING_CLIENT": "Notation de la zone de résidence",
    "REGION_RATING_CLIENT_W_CITY": "Risque de la zone d’habitation",
    "LIVINGAREA_AVG": "Niveau de logement",
    "NAME_HOUSING_TYPE_House / apartment": "Type de logement",
    "CODE_GENDER_M": "Profil démographique"
}
...
if st.button("Analyser le risque"):
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            if decision == "REFUSÉ":
                st.error("❌ Demande de crédit refusée")
            else:
            ...
            if facteurs:
                for f in facteurs:
                    nom_tech = f["feature"]
                    nom_client = TRADUCTION_FEATURES.get(
                        nom_tech,
                        "Facteur du dossier"
                    )
                    ...
                    if impact < 0:
                        st.success(f"✔ {nom_client} améliore la fiabilité du dossier")
                    else:
